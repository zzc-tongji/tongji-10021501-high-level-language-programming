...
class TString {
	...
	public:
		...
		friend void *tj_memcpy(void *obj,const void *sour,const int size);
};
...
TString& TString::operator+=(char *s)
{
	...
	if (s && s[0])
	{
		...
		if (content)
		{
			...
			{
				//strcpy(content,temp);
				tj_memcpy((void *)content,(void *)temp,len_0);
				//*(content+len_0)='\0';
			}
			//strcat(content,s);
			tj_memcpy((void *)(content+len_0),(void *)s,s_lenth);
			*(content+len_0+s_lenth)='\0';
		}
		else //申请不到空间的情况
		{
			len=0;
...
void *tj_memcpy(void *obj,const void *sour,const int size)
{
	char *read;
	char *write;
	int counter;
	...
	for (counter=0;counter<size;counter++,write++,read++)
	{
		*write=*read;
	}
